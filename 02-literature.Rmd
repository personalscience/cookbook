...
x %>% ggplot(aes(x=date,y=0)) + geom_point() + geom_vline(xintercept = x$date[x$headache], color = rainbow(1))
```
...
```{r, message = FALSE}
x %>% group_by(week = ntile(date,7)) %>% summarise(sum(headache))  %>% kable() %>% 
  kableExtra::kable_styling(bootstrap_options = c("striped", "hover", "condensed"))

x %>% group_by(week = ntile(date,7)) %>%  summarise(headaches = sum(headache)) %>%  ggplot(aes(x=week,y=headaches)) +  geom_bar(stat = "identity")
